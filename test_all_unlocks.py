...
bm_cells = get_clean_cells(14, 22)
cv_cells = get_clean_cells(22, 30)
sf_cells = get_clean_cells(30, 35)
oak_wall_cells = [(45, c) for c in range(4, 100, 8) if is_clean(45, c)]
grass_cells = get_clean_cells(60, 70)
bm_rain_cells = get_clean_cells(37, 44)
...
# Ticks 35..52: 360 Crimson Vine (4) (in rows 22..30) -> Crimson Vine > 4% (280)
...
# Tick 73: Plant 4 Purple Canopy Trees (10) in clean cells at row 30 + 16 Lavender -> Skyvine (20)!
pct_clean = [(30, c) for c in [10, 20, 30, 40] if is_clean(30, c)]
...
# Ticks 74..89: 320 Silver Fern (8) in sf_cells (rows 30..35) -> Silver Fern > 4% (280) -> Living Topiary (27)!
for t in range(74, 90):
    batch = sf_cells[(t-74)*20 : (t-73)*20]
    add(t, [{"plant_index": 8, "index": 8, "row": r, "col": c} for r, c in batch])

# Ticks 230..249: 400 Blue Moss (3) in bm_rain_cells (rows 37..44) -> Blue Moss > 5% at Tick 250 (Rain Event) -> Mire Bloom (18)!
for t in range(230, 250):
    batch = bm_rain_cells[(t-230)*20 : (t-229)*20]
    add(t, [{"plant_index": 3, "index": 3, "row": r, "col": c} for r, c in batch])

...
def generate_level2_actions():
    ...
    def get_clean_cells(r_start, r_end, c_start=0, c_end=75):
    ...
    # -------------------------------------------------------------
    # STEP 8 (Ticks 467..494): Mid-late multi-species cultivation
    # -------------------------------------------------------------
    ...
    late_species = [
        ...
        (7, get_clean_cells(11, 13), 20),    # Orange Blossom
        (5, get_clean_cells(60, 64), 20),    # Dwarf Sunflower
    ]

    cur_t = 467
    for p_idx, cells_list, target_count in late_species:
        step = max(1, len(cells_list) // target_count)
        chosen = cells_list[::step][:target_count]
        for b in range(0, len(chosen), 20):
            batch = chosen[b : b+20]
            if batch and cur_t < 495:
                add(cur_t, [{"plant_index": p_idx, "index": p_idx, "row": r, "col": c} for r, c in batch])
                cur_t += 1

    # -------------------------------------------------------------
    # STEP 9 (Ticks 495..498): Precision species balancing
    # Ensures Rose Bush, Lavender, Moonpetal Lily, Ironthorn Shrub are 100% active on Tick 500!
    # -------------------------------------------------------------
    add(495, [{"plant_index": 6, "index": 6, "row": 2, "col": c} for c in range(76, 96)])   # 20 Lavender
    add(496, [{"plant_index": 16, "index": 16, "row": 3, "col": c} for c in range(76, 96)]) # 20 Ironthorn Shrub
    add(497, [{"plant_index": 15, "index": 15, "row": 4, "col": c} for c in range(76, 96)]) # 20 Moonpetal Lily
    add(498, [{"plant_index": 2, "index": 2, "row": 5, "col": c} for c in range(20, 40)])   # 20 Rose Bush

    action_list = [{"tick": t, "plants": p[:20]} for t, p in sorted(actions_by_tick.items())]
    return {"actions": action_list}

def main():
    print("Generating Level 2 Solution...")
    sol = generate_level2_actions()
    
    os.makedirs("solutions", exist_ok=True)
    with open("solutions/level2_solution.json", "w") as f:
        json.dump(sol, f, indent=2)
    with open("solution.json", "w") as f:
        json.dump(sol, f, indent=2)

    print(f"Saved solutions/level2_solution.json and solution.json ({len(sol['actions'])} action ticks)")
    ...
    print("\n" + "="*60)
    print("LEVEL 2 SIMULATION VERIFICATION RESULTS:")
